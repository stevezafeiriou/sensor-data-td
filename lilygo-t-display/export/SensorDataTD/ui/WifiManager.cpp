    ...
    if (server.method() == HTTP_POST) {
        if (!server.hasArg("ssid") || !server.hasArg("password") || !server.hasArg("server_ip")) {
            server.send(400, "text/html", 
                ...
                "<meta charset='utf-8'><title>m+ WiFi Manager</title>"
        ...
        server.send(200, "text/html", 
            ...
            "button {cursor: pointer; border: 1px solid transparent; color: #fff; background-color: #0000ff; border-color: #0000ff; padding: .5rem 1rem; font-size: 1.25rem; line-height: 1.5; border-radius: .3rem; width: 100%;}"
